 ...
 */
    ...
    MoveAllTo(BPlusTreeInternalPage *recipient, int index_in_parent,
        ...
        for (auto index = 0; index < GetSize(); ++index) {
            auto *fetchPage = buffer_pool_manager->FetchPage(ValueAt(index));
            if (fetchPage == nullptr) {
                throw Exception(EXCEPTION_TYPE_INDEX,
                                "all fetchPage are pinned while CopyLastFrom");
            }
            auto child = reinterpret_cast<BPlusTreePage *>(fetchPage->GetData());
            child->SetParentPageId(recipient->GetPageId());
